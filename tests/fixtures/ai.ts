...
export type AIModel = 'sdxl' | 'sd-2-1' | 'dalle-3' | 'midjourney' | 'stable-diffusion-xl-lightning';
export type AspectRatio = '1:1' | '4:5' | '3:4' | '2:3' | '4:3' | '16:9' | '21:9';
export type StylePreset = 'wabi-sabi' | 'abstract-expression' | 'botanical' | 'vintage-poster' | 'minimalist' | 'geometric' | 'watercolor' | 'line-art' | 'pop-art' | 'surrealism';
...
export function createAIGeneration(overrides?: Partial<AIGeneration>): AIGeneration {
  ...
  return {
    ...
    prompt: 'A serene Japanese garden with cherry blossoms, koi pond, and traditional stone lanterns',
    enhancedPrompt: 'A serene Japanese garden with cherry blossoms in full bloom, koi pond with crystal clear water, traditional stone lanterns, morning mist, soft natural lighting, highly detailed, 8k resolution, peaceful atmosphere',
 ...
 */
export function createPendingModerationAIGeneration(overrides?: Partial<AIGeneration>): AIGeneration {
  return createAIGeneration({
    status: 'completed',
    moderationStatus: 'pending',
    isPublic: false,
...
export function createAIGenerations(userId: string, count: number = 5): AIGeneration[] {
  ...
  const statuses: AIGenerationStatus[] = ['completed', 'completed', 'completed', 'processing', 'pending'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    generations.push(createAIGeneration({
 ...
 */
export function createAIParameters(overrides?: Partial<AIGenerationParameters>): AIGenerationParameters {
  return {
    cfgScale: 7.5,
    steps: 30,
    sampler: 'DPM++ 2M Karras',
