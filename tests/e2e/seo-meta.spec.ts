...
test.describe("SEO - Global Meta Tags", () => {
  ...
  test("should have theme-color meta tag", async ({ page }) => {
    ...
    expect(themeColor).toBe("#f97316"); // MasonArt brand orange color
  });
...
test.describe("SEO - Home Page Meta Tags", () => {
  ...
  test("should have meta description with relevant keywords", async ({ page }) => {
    ...
    expect(
      lowerDesc.includes("poster") || lowerDesc.includes("art") || lowerDesc.includes("frame")
    ).toBe(true);
  });
  ...
  test("should have Twitter Card meta tags", async ({ page }) => {
    ...
    expect(twitterTitle).toContain("MasonArt");
  });
...
test.describe("SEO - Checkout Page Meta Tags", () => {
  ...
  test("should have page title with Checkout", async ({ page }) => {
    const title = await page.title();
    const lowerTitle = title.toLowerCase();
    expect(lowerTitle.includes("checkout") || lowerTitle.includes("masonart")).toBe(true);
  });
...
test.describe("SEO - Meta Tags Best Practices", () => {
  ...
  for (const pageUrl of publicPages) {
    ...
    test(`${pageUrl} - should have description between 50-160 characters`, async ({ page }) => {
      await page.goto(pageUrl);
      const description = await getMetaByName(page, "description");
      expect(description).toBeTruthy();
    ...
    test(`${pageUrl} - should have unique title different from description`, async ({ page }) => {
      await page.goto(pageUrl);
      const title = await page.title();
      const description = await getMetaByName(page, "description");
      expect(description).toBeTruthy();
...
test.describe("SEO - Meta Tag Consistency", () => {
  ...
  test("meta description and og:description should be related", async ({ page }) => {
    ...
    expect(descLower.includes("poster") || descLower.includes("art")).toBe(true);
    expect(ogDescLower.includes("poster") || ogDescLower.includes("art")).toBe(true);
  });
});
