 ...
 * Wallet E2E Tests
 *
 * Tests for the MasonArt wallet management page (/account/wallet).
 *
...
test.describe("Wallet Page - Transaction History", () => {
  ...
  test("should show empty state or transactions", async ({ page }) => {
    ...
    const transactionOrEmpty = page
      .locator("text=/No transactions|credit|debit|top.?up|generation/i")
      .first();
...
test.describe("Wallet Page - Top-up Flow", () => {
  ...
  test("should validate minimum amount for custom top-up", async ({ page }) => {
    ...
    const addButton = page
      .locator("button")
      .filter({ hasText: /Add|Top.?up/i })
      .last();
    await addButton.click();

    // Should show error about minimum amount (target the red error message specifically)
    const errorMessage = page
      .locator(".text-red-800, .text-red-600, .text-destructive")
      .filter({ hasText: /minimum/i })
      .first();
  ...
  test("clicking top-up button should attempt to load Razorpay", async ({ page }) => {
    ...
    const anyResponse = page
      .locator("text=/Processing|Loading|Opening|Payment|failed|error/i")
      .first();
...
test.describe("AI Generator - Cost Preview", () => {
  ...
  test("should show cost preview on create page when logged in", async ({ page }) => {
    ...
    const costIndicator = page.locator("text=/₹|cost|price|free|generation|balance/i").first();
