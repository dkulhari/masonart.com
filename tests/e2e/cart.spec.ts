 ...
 * Cart Page E2E Tests
 *
 * Tests for the MasonArt cart page (/cart) including:
...
test.describe("Cart Page - Empty Cart State", () => {
  ...
  test("should display View Featured Collection link", async ({ page }) => {
    const featuredLink = page.locator(
      'a[href="/posters?featured=true"]:has-text("View Featured Collection")'
    );
...
test.describe("Cart Page - Clear Cart", () => {
  ...
  test("should remove all items from localStorage", async ({ page }) => {
    ...
    const cartData = await page.evaluate(() => {
      const data = localStorage.getItem("masonart-cart-storage");
