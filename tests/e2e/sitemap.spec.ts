...
/**
 * API base URL where sitemap.xml is served
 * The sitemap is generated by the API server, not the web server.
 */
...
async function fetchSitemap(
  ...
  const response = await page.goto(`${API_BASE_URL}/sitemap.xml`);
...
test.describe('Sitemap - Performance', () => {
  ...
  test('sitemap should load without JavaScript', async ({ page }) => {
    ...
    if (noJsContext) {
      ...
      const response = await noJsPage.goto(`${API_BASE_URL}/sitemap.xml`);
