...
test.describe("Admin Reviews - Reviews Table", () => {
  ...
  test("should display table with headers or no reviews message", async ({ page }) => {
    ...
    await expect(page.locator('table, :text("No reviews found")').first()).toBeVisible({
      timeout: 10000,
    });
...
test.describe("Admin Reviews - Review Row", () => {
  ...
  test("review row should display star rating", async ({ page }) => {
    const stars = page
      .locator('tbody svg[class*="fill-amber"], tbody [class*="text-amber"]')
      .first();
  ...
  test("review row should display status badge", async ({ page }) => {
    const statusBadge = page
      .locator(
        'tbody span:has-text("pending"), tbody span:has-text("approved"), tbody span:has-text("rejected")'
      )
      .first();
...
test.describe("Admin Reviews - Bulk Actions", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .getByRole("heading", { name: /Reviews/i })
