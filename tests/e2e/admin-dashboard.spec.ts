...
test.describe("Key Metrics Grid", () => {
  ...
  test("should display formatted total revenue amount", async ({ page }) => {
    ...
    const revenueCard = page
      .locator('[class*="StatsCard"], .rounded-xl')
...
test.describe("Order Status Breakdown", () => {
  ...
  test("should display color indicators for each status", async ({ page }) => {
    ...
    const colorDots = page.locator(
      ".rounded-full.bg-amber-500, .rounded-full.bg-blue-500, .rounded-full.bg-indigo-500, .rounded-full.bg-green-500, .rounded-full.bg-red-500"
    );
