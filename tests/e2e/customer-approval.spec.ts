  ...
  comments: [
    {
      ...
      authorType: "admin",
      comment:
        "Your production photos are ready for review. Please check the color accuracy and print quality.",
...
test.describe("Customer Approval - Pending Approval State", () => {
  ...
  test("displays order item information", async ({ page }) => {
    await expect(
      page.getByText("Custom AI Poster - Mountain Sunset", { exact: true })
    ).toBeVisible();
...
test.describe("Customer Approval - Request Changes", () => {
  ...
  test("can submit change request with comment", async ({ page }) => {
    ...
    // Fill in comment
    await page
      .getByPlaceholder(/describe the changes/i)
      .fill("Please adjust the brightness of the image.");
