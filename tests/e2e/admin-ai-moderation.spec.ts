 ...
 *
 * Tests for the MasonArt admin AI content moderation dashboard including:
 * - Access control
 * - Stats display
...
test.describe("Admin AI Moderation - Stats Cards", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .locator('h1:has-text("AI Moderation")')
...
test.describe("Admin AI Moderation - Filter Controls", () => {
  ...
  test("should display status filter dropdown", async ({ page }) => {
    const statusFilter = page
      .locator('select, button:has-text("Status"), [aria-label="Status filter"]')
      .first();
    await expect(statusFilter).toBeVisible();
  });

  test("should display style preset filter", async ({ page }) => {
    const styleFilter = page
      .locator('select, button:has-text("Style"), [aria-label="Style filter"]')
      .first();
...
test.describe("Admin AI Moderation - Generations Grid", () => {
  ...
  test("should display grid of generations or empty state", async ({ page }) => {
    ...
    const gridItems = page.locator(
      '[data-testid="generation-card"], .generation-card, article, .rounded-xl.border'
    );
    const emptyMessage = page.locator("text=No generations, text=no items");

    const hasGridItems = await gridItems
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test("generation card should display image or placeholder", async ({ page }) => {
    ...
    const image = page
      .locator('img[src*="generation"], img[alt*="generation"], img[alt*="AI"]')
      .first();
  ...
  test("generation card should display style preset badge", async ({ page }) => {
    ...
    const styleBadge = page
      .locator(
        'span:has-text("Wabi"), span:has-text("Abstract"), span:has-text("Botanical"), [data-testid="style-badge"]'
      )
      .first();
    const emptyMessage = page.locator("text=No generations");
...
test.describe("Admin AI Moderation - Bulk Actions", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for page to load
    await page
      .getByRole("heading", { name: /AI Moderation/i })
      .waitFor({ state: "visible", timeout: 10000 });
  });
  ...
  test("bulk bar should have bulk approve button", async ({ page }) => {
    ...
    if (await checkbox.isVisible({ timeout: 3000 }).catch(() => false)) {
      await checkbox.click();
      const bulkApproveBtn = page.locator(
        'button:has-text("Bulk Approve"), button:has-text("Approve All")'
      );
  ...
  test("bulk bar should have bulk reject button", async ({ page }) => {
    ...
    if (await checkbox.isVisible({ timeout: 3000 }).catch(() => false)) {
      await checkbox.click();
      const bulkRejectBtn = page.locator(
        'button:has-text("Bulk Reject"), button:has-text("Reject All")'
      );
...
test.describe("Admin AI Moderation - Image Preview Modal", () => {
  ...
  test("clicking on generation image should open preview modal", async ({ page }) => {
    ...
    const image = page
      .locator('img[src*="generation"], img[alt*="generation"], article img')
      .first();
  ...
  test("rejection modal should have category dropdown", async ({ page }) => {
    ...
    if (await rejectBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const categoryDropdown = page.locator(
        'select, [role="combobox"], button:has-text("Category")'
      );
...
test.describe("Admin AI Moderation - Pagination", () => {
  ...
  test("should display pagination when multiple pages", async ({ page }) => {
    ...
    const pagination = page.locator(
      'button:has-text("Next"), button:has-text("Previous"), text=Page'
    );
...
test.describe("Admin AI Moderation - Navigation", () => {
  ...
  test("admin sidebar should have AI moderation link", async ({ page }) => {
    ...
    const moderationLink = page.locator(
      'a[href="/admin/ai-moderation"], a:has-text("AI Moderation"), a:has-text("AI Content")'
    );
