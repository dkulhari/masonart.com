 ...
 * Home Page E2E Tests
 *
 * Tests for the chobii.art home page including:
...
test.describe('Home Page - Value Propositions Section', () => {
  ...
  test('should display Why Choose chobii.art section header', async ({ page }) => {
    const header = page.locator('h2:has-text("Why Choose chobii.art")');
    await expect(header).toBeVisible();
  });
...
test.describe('Home Page - Responsive Design', () => {
  ...
  test('should display all sections on tablet', async ({ page }) => {
    ...
    await expect(page.locator('h2:has-text("Why Choose chobii.art")')).toBeVisible();
