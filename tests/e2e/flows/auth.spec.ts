 ...
 */
async function mockSuccessfulSignIn(
  page: Page,
  userData?: Partial<{
    id: string;
    name: string;
...
test.describe("Auth Flow - Sign Out Journey", () => {
  ...
  test.skip("should clear session and require re-authentication after sign out", async ({
    page,
  }) => {
    // Skipped: SSR session mocking not supported with Playwright's route()
  });
...
test.describe("Auth Flow - Performance", () => {
  ...
  // Skipped: mockAuthenticatedSession doesn't work with server-side session validation
  test.skip("should load account page within acceptable time when authenticated", async ({
    page,
  }) => {
    await mockAuthenticatedSession(page);
    await mockEmptyOrders(page);
