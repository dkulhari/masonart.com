 ...
 */
async function fillValidAddressForm(
  page: Page,
  overrides?: Partial<{
    fullName: string;
    email: string;
...
async function setupShippingMock(page: Page, cartTotal = 2999) {
  await page.route("**/api/shipping/estimate*", async (route) => {
    ...
    const stdMin = new Date(today);
    stdMin.setDate(stdMin.getDate() + 5);
    const stdMax = new Date(today);
    stdMax.setDate(stdMax.getDate() + 7);
    const expMin = new Date(today);
    expMin.setDate(expMin.getDate() + 2);
    const expMax = new Date(today);
    expMax.setDate(expMax.getDate() + 3);
...
test.describe("Checkout Flow - Product to Cart", () => {
  ...
  test("should add product to cart from product detail page", async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await expect(
        page
          .locator('h2:has-text("Your cart is empty")')
          .or(page.locator("text=/\\d+ items? in your cart/"))
      ).toBeVisible();
    }
  });
...
test.describe.skip("Checkout Flow - Complete Purchase", () => {
  ...
  test("should initiate payment when clicking Pay button", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Creating Order").or(page.locator("button .animate-spin")).first()
