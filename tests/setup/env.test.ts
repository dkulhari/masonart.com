...
const isTestMode =
  process.env.CI === "true" || process.env.NODE_ENV === "test" || !process.env.ENFORCE_ENV_VARS;
  ...
  describe("Environment Variable Security", () => {
    it("should not have empty required environment variables", () => {
      ...
      if (isTestMode && emptyVars.length > 0) {
        console.warn(
          `⚠️  ${emptyVars.length} required environment variables not set (skipped in test mode)`
        );
        expect(true).toBe(true);
      } else {
  ...
  describe("Environment Variable Summary", () => {
    it("should have all required environment variables present", () => {
      ...
      if (missingVars.length > 0) {
        if (isTestMode) {
          console.warn(
            "⚠️  Missing environment variables (skipped in test mode):",
            missingVars.join(", ")
          );
