...
- **PASS**: Page loads with title "Shop Posters | chobii.art", heading and subheading render, filter sidebar renders with Sort By, Orientation, Style (10 options), Subject (9 options), Color, Room, Special sections, mobile filter button present
...
- **PASS**: All page titles correct and unique, meta descriptions on all pages, OG tags (title, description, type=website, site_name=chobii.art), Twitter cards (summary_large_image), theme-color #f97316, viewport on all 6 tested pages, robots.txt properly configured, /posters has canonical + full og:image, /checkout and /cart have noindex, DOCTYPE on all pages, html lang="en" on all pages, /create has page-specific OG title/description, /gallery has page-specific OG title/description
