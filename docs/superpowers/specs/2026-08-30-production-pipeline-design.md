...
| **A cancelled job strands a phantom payable** — five sites filtered on `settlement_id IS NULL` alone (`vendor-payables.ts` ×2, `getVendorPayableTotal`, `vendors.ts` `amountOwed`, and the vendor portal's own "You are paid" column). Cancellation becomes reachable in this feature, so this went live here. **The fix this table originally stated — `status <> 'cancelled'` — was wrong** and would have silently deleted agreed kill fees, contradicting §10.6 and the lost-transfer decision. Shipped predicate: `settlement_id IS NULL AND (status <> 'cancelled' OR amount_actual IS NOT NULL)` — on a cancelled job the rate card's `amount_expected` is a guess, not a debt; only an `amount_actual` an admin stated is owed. The rule now has one owner in `lib/vendor-payables`, in both a JS and a SQL form, with `status` required on `PayableJob` so a new call site cannot forget it | Fixed in #695 |
