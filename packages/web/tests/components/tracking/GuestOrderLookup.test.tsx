  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6" data-testid="guest-lookup-form">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Error Message */}
        {displayError && (
          <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 p-3" role="alert">
...
describe('GuestOrderLookupForm', () => {
  ...
  describe('Loading State', () => {
    ...
    it('disables contact method toggle during loading', () => {
      ...
      const emailButton = screen.getAllByRole('button').find(
        (btn) => btn.textContent?.includes('Email')
      )
      const phoneButton = screen.getAllByRole('button').find(
        (btn) => btn.textContent?.includes('Phone')
      )
