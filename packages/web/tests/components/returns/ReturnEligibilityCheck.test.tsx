...
describe("ReturnEligibilityCheck Component", () => {
  describe("Loading State", () => {
    it("shows loading skeleton when isLoading is true", () => {
      const { container } = render(<ReturnEligibilityCheck isEligible={false} isLoading />);
  ...
  describe("Not Eligible State", () => {
    ...
    it("shows custom message when provided", () => {
      render(<ReturnEligibilityCheck isEligible={false} message="Order must be delivered first" />);
  ...
  describe("Styling", () => {
    it("applies custom className", () => {
      const { container } = render(<ReturnEligibilityCheck isEligible className="custom-class" />);

      expect(container.firstChild).toHaveClass("custom-class");
    });

    it("uses green styling for eligible state", () => {
      const { container } = render(<ReturnEligibilityCheck isEligible daysRemaining={10} />);

      expect(container.querySelector('[class*="green"]')).toBeInTheDocument();
    });

    it("uses amber styling for urgent state", () => {
      const { container } = render(<ReturnEligibilityCheck isEligible daysRemaining={2} />);
