...
export function FrameSelector({
  ...
  if (availableFrames.length === 0) {
    return <div className={cn("text-muted-foreground", className)}>No frame options available</div>;
  }
...
function FrameOptionCard({ frame, isSelected, priceDisplay, onClick }: FrameOptionCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        {frame.imageUrl ? (
          <img src={frame.imageUrl} alt="" className="h-full w-full rounded object-cover" />
        ) : (
        ...
        <div className="mt-1 flex items-center gap-2">
          ...
          {frame.material && (
            <>
              ...
              <span className="text-xs text-muted-foreground truncate">{frame.material}</span>
            </>
          )}
        </div>
      </div>
