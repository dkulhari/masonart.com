/**
 * Add Funds Button Component - MasonArt E-commerce Platform
 *
...
export function AddFundsButton({
  ...
  const IconComponent = () => {
    if (isLoading)
      return (
        <Loader2 className={cn("animate-spin", variant === "compact" ? "h-3 w-3" : "h-4 w-4")} />
      );
    if (status === "success")
      return <CheckCircle className={variant === "compact" ? "h-3 w-3" : "h-4 w-4"} />;
    if (status === "error")
      return <AlertCircle className={variant === "compact" ? "h-3 w-3" : "h-4 w-4"} />;
