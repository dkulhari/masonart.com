...
async function fetchRecentOrders(): Promise<RecentOrder[]> {
  const response = await fetch(
    `${getApiUrl()}/api/admin/orders?pageSize=5&sortBy=createdAt&sortOrder=desc`,
    {
      method: "GET",
      credentials: "include",
      headers: {
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card">
            ...
            <div className="p-2">
              ...
              />
              <QuickActionButton href="/admin/orders" icon={ShoppingCart} label="View All Orders" />
              <QuickActionButton href="/admin/customers" icon={Users} label="Manage Customers" />
              <QuickActionButton href="/" icon={ExternalLink} label="View Store" external />
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="text-right">
        <p className="font-semibold text-foreground">{formatPrice(parseFloat(order.total))}</p>
        <p
          className={cn(
            "text-xs capitalize",
...
function QuickActionButton({ href, icon: Icon, label, external }: QuickActionButtonProps) {
  return (
    <a
      href={href}
