...
const searchParamsSchema = z.object({
  ...
  status: z.enum(['pending_upload', 'pending_approval', 'changes_requested', 'approved', 'expired']).optional(),
...
function ApprovalCard({ approval }: { approval: Approval }) {
  ...
  const getDeadlineStatus = () => {
    ...
    if (hoursLeft < 24) return { text: `${Math.round(hoursLeft)}h left`, className: 'text-orange-600' }
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className={cn('rounded-full px-2 py-1 text-xs font-medium', statusColors[approval.status])}>
...
function AdminApprovalsPage() {
  const search = Route.useSearch() as { page?: number; pageSize?: number; status?: string; search?: string }
          ...
          {statsLoading ? (
            ...
            <>
              ...
              <StatsCard
                ...
                trendLabel={stats.byStatus.changes_requested > 0 ? 'Needs attention' : 'None pending'}
              />
              <StatsCard
