...
describe('AI Generation Parameters Schema', () => {
  ...
  describe('Negative prompt validation', () => {
    it('should accept negative prompts up to 1000 characters', () => {
      expect(AIGenerationParametersSchema.safeParse({ negativePrompt: 'A'.repeat(1000) }).success).toBe(true);
    });

    it('should reject negative prompts over 1000 characters', () => {
      expect(AIGenerationParametersSchema.safeParse({ negativePrompt: 'A'.repeat(1001) }).success).toBe(false);
    });
  });
});
...
describe('AI Generation Image Schema', () => {
  ...
  describe('Image URL validation', () => {
    ...
    it('should reject invalid URLs', () => {
      expect(AIGenerationImageSchema.safeParse({ ...validImage, url: 'not-a-url' }).success).toBe(false);
    });
  });

  describe('Dimensions validation', () => {
    it('should accept positive integer dimensions', () => {
      expect(AIGenerationImageSchema.safeParse({ ...validImage, width: 512, height: 768 }).success).toBe(true);
      expect(AIGenerationImageSchema.safeParse({ ...validImage, width: 2048, height: 2048 }).success).toBe(true);
    });
    ...
    it('should reject negative dimensions', () => {
      expect(AIGenerationImageSchema.safeParse({ ...validImage, width: -1024 }).success).toBe(false);
      expect(AIGenerationImageSchema.safeParse({ ...validImage, height: -1024 }).success).toBe(false);
    });

    it('should reject non-integer dimensions', () => {
      expect(AIGenerationImageSchema.safeParse({ ...validImage, width: 1024.5 }).success).toBe(false);
      expect(AIGenerationImageSchema.safeParse({ ...validImage, height: 1024.5 }).success).toBe(false);
    });
  });
});
...
describe('AI Generation Schema', () => {
  const validGeneration = {
    ...
    enhancedPrompt: 'A serene ocean landscape with minimalist aesthetic, wabi-sabi style, muted earth tones',
  ...
  it('should validate generation without optional fields', () => {
    const { enhancedPrompt, parameters, selectedImageId, errorMessage, processingTimeMs, creditsUsed, moderationNotes, moderatedBy, moderatedAt, completedAt, ...generation } = validGeneration;
  ...
  describe('Prompt validation', () => {
    it('should accept prompts between 3 and 1000 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, prompt: 'cat' }).success).toBe(true);
      expect(AIGenerationSchema.safeParse({ ...validGeneration, prompt: 'A'.repeat(1000) }).success).toBe(true);
    });

    it('should reject prompts under 3 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, prompt: 'ab' }).success).toBe(false);
    });

    it('should reject prompts over 1000 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, prompt: 'A'.repeat(1001) }).success).toBe(false);
    });
  });

  describe('Enhanced prompt validation', () => {
    it('should accept enhanced prompts up to 2000 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, enhancedPrompt: 'A'.repeat(2000) }).success).toBe(true);
    });

    it('should reject enhanced prompts over 2000 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, enhancedPrompt: 'A'.repeat(2001) }).success).toBe(false);
    });
  });
  ...
  describe('Error message validation', () => {
    it('should accept error messages up to 500 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, errorMessage: 'A'.repeat(500) }).success).toBe(true);
    });

    it('should reject error messages over 500 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, errorMessage: 'A'.repeat(501) }).success).toBe(false);
    });
  });

  describe('Moderation notes validation', () => {
    it('should accept moderation notes up to 1000 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, moderationNotes: 'A'.repeat(1000) }).success).toBe(true);
    });

    it('should reject moderation notes over 1000 characters', () => {
      expect(AIGenerationSchema.safeParse({ ...validGeneration, moderationNotes: 'A'.repeat(1001) }).success).toBe(false);
    });
  });
...
describe('Style Preset Detail Schema', () => {
  ...
  describe('Example images validation', () => {
    it('should accept up to 5 example images', () => {
      ...
      expect(StylePresetDetailSchema.safeParse({ ...validPreset, exampleImages: images }).success).toBe(true);
    });

    it('should reject more than 5 example images', () => {
      const images = Array(6).fill('https://cdn.example.com/example.jpg');
      expect(StylePresetDetailSchema.safeParse({ ...validPreset, exampleImages: images }).success).toBe(false);
    });
  });

  describe('Prompt modifiers validation', () => {
    it('should accept prompt modifiers up to 1000 characters', () => {
      expect(StylePresetDetailSchema.safeParse({ ...validPreset, promptModifiers: 'A'.repeat(1000) }).success).toBe(true);
    });

    it('should reject prompt modifiers over 1000 characters', () => {
      expect(StylePresetDetailSchema.safeParse({ ...validPreset, promptModifiers: 'A'.repeat(1001) }).success).toBe(false);
    });
  });
});
...
describe('AI Generation Stats Schema', () => {
  ...
  it('should reject negative values', () => {
    expect(AIGenerationStatsSchema.safeParse({ ...validStats, totalGenerations: -1 }).success).toBe(false);
    expect(AIGenerationStatsSchema.safeParse({ ...validStats, averageProcessingTimeMs: -1 }).success).toBe(false);
  });
});
...
describe('AI Image Selection Schema', () => {
  ...
  it('should reject empty generation ID', () => {
    expect(AIImageSelectionSchema.safeParse({ ...validSelection, generationId: '' }).success).toBe(false);
  });

  it('should reject invalid image URL', () => {
    expect(AIImageSelectionSchema.safeParse({ ...validSelection, imageUrl: 'not-a-url' }).success).toBe(false);
  });
});
...
describe('AI Generation Regenerate Schema', () => {
  ...
  it('should reject empty generation ID', () => {
    expect(AIGenerationRegenerateSchema.safeParse({ ...validRegenerate, generationId: '' }).success).toBe(false);
  });

  describe('Modified prompt validation', () => {
    it('should accept prompts between 3 and 1000 characters', () => {
      expect(AIGenerationRegenerateSchema.safeParse({ ...validRegenerate, modifiedPrompt: 'cat' }).success).toBe(true);
      expect(AIGenerationRegenerateSchema.safeParse({ ...validRegenerate, modifiedPrompt: 'A'.repeat(1000) }).success).toBe(true);
    });

    it('should reject prompts under 3 characters', () => {
      expect(AIGenerationRegenerateSchema.safeParse({ ...validRegenerate, modifiedPrompt: 'ab' }).success).toBe(false);
    });

    it('should reject prompts over 1000 characters', () => {
      expect(AIGenerationRegenerateSchema.safeParse({ ...validRegenerate, modifiedPrompt: 'A'.repeat(1001) }).success).toBe(false);
    });
  });
});
