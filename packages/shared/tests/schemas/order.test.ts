...
describe("Address Schema", () => {
  ...
  describe("Full name validation", () => {
    it("should accept names with 2-100 characters", () => {
      ...
      expect(AddressSchema.safeParse({ ...validAddress, fullName: "A".repeat(100) }).success).toBe(
        true
      );
    });
  ...
  describe("Phone validation", () => {
    it("should accept valid E.164 phone numbers", () => {
      const validPhones = ["+919876543210", "+14155552671", "+442071838750", "+861082222222"];
  ...
  describe("Address line validation", () => {
    it("should accept address line 1 with 5-200 characters", () => {
      expect(AddressSchema.safeParse({ ...validAddress, addressLine1: "12345" }).success).toBe(
        true
      );
      expect(
        AddressSchema.safeParse({ ...validAddress, addressLine1: "A".repeat(200) }).success
      ).toBe(true);
    });
    ...
    it("should accept optional address line 2 up to 200 characters", () => {
      expect(
        AddressSchema.safeParse({ ...validAddress, addressLine2: "A".repeat(200) }).success
      ).toBe(true);
    });
  ...
  describe("City and state validation", () => {
    it("should accept city and state with 2-100 characters", () => {
      expect(AddressSchema.safeParse({ ...validAddress, city: "AB", state: "CD" }).success).toBe(
        true
      );
      expect(
        AddressSchema.safeParse({ ...validAddress, city: "A".repeat(100), state: "B".repeat(100) })
          .success
      ).toBe(true);
    });
    ...
    it("should reject state over 100 characters", () => {
      expect(AddressSchema.safeParse({ ...validAddress, state: "A".repeat(101) }).success).toBe(
        false
      );
    });
  });
...
describe("Saved Address Schema", () => {
  ...
  it("should require a valid UUID for id", () => {
    expect(SavedAddressSchema.safeParse({ ...validSavedAddress, id: "not-a-uuid" }).success).toBe(
      false
    );
  });
  ...
  it("should validate countryCode as 2 characters", () => {
    expect(SavedAddressSchema.safeParse({ ...validSavedAddress, countryCode: "IN" }).success).toBe(
      true
    );
    expect(SavedAddressSchema.safeParse({ ...validSavedAddress, countryCode: "IND" }).success).toBe(
      false
    );
  });
...
describe("Saved Address Create Schema", () => {
  ...
  it("should reject invalid postal code", () => {
    expect(SavedAddressCreateSchema.safeParse({ ...validCreate, postalCode: "1234" }).success).toBe(
      false
    );
  });
});
...
describe("Saved Address Update Schema", () => {
  ...
  it("should still validate field constraints", () => {
    expect(SavedAddressUpdateSchema.safeParse({ fullName: "A" }).success).toBe(false);
    expect(SavedAddressUpdateSchema.safeParse({ postalCode: "ABC" }).success).toBe(false);
  });
});
...
describe("Order Item Schema", () => {
  ...
  describe("Price validation", () => {
    it("should accept valid price formats", () => {
      ...
      validPrices.forEach((price) => {
        expect(OrderItemSchema.safeParse({ ...validOrderItem, unitPrice: price }).success).toBe(
          true
        );
        expect(OrderItemSchema.safeParse({ ...validOrderItem, subtotal: price }).success).toBe(
          true
        );
      });
    });
  ...
  describe("Image URL validation", () => {
    ...
    it("should reject invalid URLs", () => {
      expect(OrderItemSchema.safeParse({ ...validOrderItem, imageUrl: "not-a-url" }).success).toBe(
        false
      );
    });
  });
...
describe("Order Schema", () => {
  ...
  describe("Order number validation", () => {
    ...
    it("should reject order number with lowercase", () => {
      expect(OrderSchema.safeParse({ ...validOrder, orderNumber: "ord-2024-001" }).success).toBe(
        false
      );
    });
