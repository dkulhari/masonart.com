...
describe('Trade Business Schema', () => {
  ...
  describe('Business name validation', () => {
    it('should accept names with 2-200 characters', () => {
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessName: 'AB' }).success).toBe(true);
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessName: 'A'.repeat(200) }).success).toBe(true);
    });

    it('should reject names under 2 characters', () => {
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessName: 'A' }).success).toBe(false);
    });

    it('should reject names over 200 characters', () => {
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessName: 'A'.repeat(201) }).success).toBe(false);
    });
  });
  ...
  describe('GST number validation', () => {
    it('should accept valid GST numbers', () => {
      ...
      validGSTs.forEach(gstNumber => {
        expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, gstNumber }).success).toBe(true);
      });
    });
    ...
    it('should reject invalid GST number formats', () => {
      ...
      invalidGSTs.forEach(gstNumber => {
        expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, gstNumber }).success).toBe(false);
      });
    });
  });
  ...
  describe('Business type validation', () => {
    it('should accept types with 2-100 characters', () => {
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessType: 'AB' }).success).toBe(true);
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessType: 'A'.repeat(100) }).success).toBe(true);
    });

    it('should reject types under 2 characters', () => {
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessType: 'A' }).success).toBe(false);
    });

    it('should reject types over 100 characters', () => {
      expect(TradeBusinessSchema.safeParse({ ...validTradeBusiness, businessType: 'A'.repeat(101) }).success).toBe(false);
    });
  });
});
...
describe('User Create Schema', () => {
  ...
  describe('Password validation', () => {
    ...
    it('should reject passwords under 8 characters', () => {
      expect(UserCreateSchema.safeParse({ ...validUserCreate, password: 'Pass1' }).success).toBe(false);
    });

    it('should reject passwords over 100 characters', () => {
      const longPassword = 'A'.repeat(50) + 'a'.repeat(50) + '1';
      expect(UserCreateSchema.safeParse({ ...validUserCreate, password: longPassword }).success).toBe(false);
    });

    it('should reject passwords without lowercase letter', () => {
      expect(UserCreateSchema.safeParse({ ...validUserCreate, password: 'PASSWORD123' }).success).toBe(false);
    });
    ...
    it('should reject passwords without number', () => {
      expect(UserCreateSchema.safeParse({ ...validUserCreate, password: 'PasswordABC' }).success).toBe(false);
    });
  });
});
...
describe('Password Reset Schema', () => {
  ...
  it('should apply same password rules as user creation', () => {
    ...
    // No uppercase
    expect(PasswordResetSchema.safeParse({ ...validReset, password: 'password123' }).success).toBe(false);
    // No lowercase
    expect(PasswordResetSchema.safeParse({ ...validReset, password: 'PASSWORD123' }).success).toBe(false);
    // No number
    expect(PasswordResetSchema.safeParse({ ...validReset, password: 'PasswordABC' }).success).toBe(false);
  });
});
