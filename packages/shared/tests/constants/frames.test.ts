...
describe('Price modifier structures', () => {
  ...
  it('should have ascending prices for glass options', () => {
    expect(STANDARD_GLASS.priceModifier.value).toBe(0);
    expect(NON_GLARE_GLASS.priceModifier.value).toBeLessThan(
      ACRYLIC_GLASS.priceModifier.value
    );
    expect(ACRYLIC_GLASS.priceModifier.value).toBeLessThan(
      MUSEUM_GLASS.priceModifier.value
    );
  });
});
