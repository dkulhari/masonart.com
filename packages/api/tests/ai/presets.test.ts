...
describe('STYLE_PRESETS Configuration', () => {
    ...
    expectedStylePresets.forEach((presetId) => {
      describe(`${presetId} preset`, () => {
        ...
        it('should have valid preferredProvider', () => {
          const validProviders: AIModelProvider[] = ['stable-diffusion', 'dall-e-3', 'midjourney', 'fal-ai'];
  ...
  describe('Category distribution', () => {
    ...
    it('should have presets in illustrative category', () => {
      const illustrativePresets = Object.values(STYLE_PRESETS).filter((p) => p.category === 'illustrative');
      expect(illustrativePresets.length).toBeGreaterThanOrEqual(1);
    });

    it('should have presets in decorative category', () => {
      const decorativePresets = Object.values(STYLE_PRESETS).filter((p) => p.category === 'decorative');
      expect(decorativePresets.length).toBeGreaterThanOrEqual(1);
    });

    it('should have presets in photographic category', () => {
      const photographicPresets = Object.values(STYLE_PRESETS).filter((p) => p.category === 'photographic');
...
describe('COLOR_MOODS Configuration', () => {
  const expectedColorMoods = ['warm', 'cool', 'neutral', 'vibrant', 'muted', 'monochrome', 'earth-tones', 'pastel'];
...
describe('constructEnhancedPrompt', () => {
  describe('Basic prompt construction', () => {
    it('should return user prompt if style preset is invalid', () => {
      const result = constructEnhancedPrompt('A beautiful sunset', 'invalid-preset' as AIStylePreset);
  ...
  describe('With color mood', () => {
    ...
    it('should ignore invalid color mood', () => {
      const result = constructEnhancedPrompt('A forest scene', 'botanical', { colorMood: 'invalid-mood' });
...
describe('isValidColorMood', () => {
  describe('Valid color moods', () => {
    const validMoods = ['warm', 'cool', 'neutral', 'vibrant', 'muted', 'monochrome', 'earth-tones', 'pastel'];
