  ...
  describe("GET /api/admin/orders - Search and Date Filters", () => {
    ...
    it("should accept combined query parameters", async () => {
      ...
      const res = await app.request(
        "/api/admin/orders?page=1&pageSize=20&status=pending&paymentStatus=paid&search=test&sortBy=createdAt&sortOrder=desc"
      );
      expect(res.status).toBe(401);
    });
  });
  ...
  describe("POST /api/admin/orders/:id/refund", () => {
    it("should require reason field", async () => {
      ...
      const res = await app.request(`/api/admin/orders/${validOrderId}/refund`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: 1999.0 }),
      });
      expect([400, 401].includes(res.status)).toBe(true);
    });
...
describe("Admin Orders Performance Tests", () => {
  ...
  it("should handle concurrent requests gracefully", async () => {
    ...
    const requests = Array.from({ length: 5 }, () => app!.request("/api/admin/orders"));
...
describe("Admin Orders Number Format", () => {
  ...
  it("should accept order numbers from different years", () => {
    const years = ["2023", "2024", "2025", "2026"];
    years.forEach((year) => {
      const orderNumber = `MA-${year}-000001`;
      const isValid = /^MA-\d{4}-\d{6}$/.test(orderNumber);
      expect(isValid).toBe(true);
    });
  });
