 ...
 */
tracking.get("/lookup", zValidator("query", guestLookupSchema), async (c) => {
  ...
  try {
    ...
    const phoneMatches =
      phone && orderPhone && normalizePhone(phone) === normalizePhone(orderPhone);
    ...
    return c.json({ error: "Failed to look up order", code: "LOOKUP_ERROR" }, 500);
  }
});
 ...
 */
tracking.get("/token/:token", async (c) => {
  const token = c.req.param("token");

  if (!token || token.length < 32) {
    return c.json({ error: "Invalid tracking token", code: "INVALID_TOKEN" }, 400);
  }
  ...
  try {
    ...
    if (!order) {
      return c.json({ error: "Order not found or link expired", code: "TOKEN_NOT_FOUND" }, 404);
    }

    // Check if token has expired
    if (order.trackingTokenExpiresAt && new Date(order.trackingTokenExpiresAt) < new Date()) {
      return c.json({ error: "This tracking link has expired", code: "TOKEN_EXPIRED" }, 410);
    }
