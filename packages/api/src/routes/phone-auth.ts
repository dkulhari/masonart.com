 ...
 */
phoneAuthApp.post("/verify-otp", zValidator("json", verifyOTPSchema), async (c) => {
  ...
  try {
    ...
    await db.delete(verifications).where(eq(verifications.identifier, `phone:${normalizedPhone}`));
 ...
 */
phoneAuthApp.post("/resend-otp", zValidator("json", sendOTPSchema), async (c) => {
  const { phone } = c.req.valid("json");
  ...
  try {
    ...
    if (existingVerification) {
      ...
      await db.delete(verifications).where(eq(verifications.id, existingVerification.id));
    }
