...
adminWalletConfigApp.get("/", async (c) => {
  try {
    ...
    const configMap = new Map<
      string,
      (typeof configs)[0] & { defaultValue: number }
    >();
    ...
    for (const config of configs) {
      if (!configMap.has(config.key)) {
        configMap.set(config.key, {
          ...
          defaultValue:
            WALLET_CONFIG_DEFAULTS[
              config.key as keyof typeof WALLET_CONFIG_DEFAULTS
            ] ?? 0,
        });
      }
    }
...
adminWalletConfigApp.put(
  "/:key",
  zValidator("json", updateConfigSchema),
  async (c) => {
    const user = c.get("user");
    const { key } = c.req.param();
    const { valueInt, description, effectiveFrom, effectiveTo } =
      c.req.valid("json");
      ...
      await db
        ...
        .where(
          and(
            ...
            sql`${walletPricingConfig.effectiveTo} IS NULL`
          )
        );
...
adminWalletConfigApp.get(
  ...
  async (c) => {
    ...
    try {
      ...
      const completedStats = transactionStats.filter(
        (s) => s.status === "completed"
      );
      const totalCredits =
        completedStats.find((s) => s.type === "credit")?.totalPaise ?? 0;
      const totalDebits =
        completedStats.find((s) => s.type === "debit")?.totalPaise ?? 0;
      const totalRefunds =
        completedStats.find((s) => s.type === "refund")?.totalPaise ?? 0;
...
function getConfigDescription(key: string): string {
  const descriptions: Record<string, string> = {
    ...
    [WALLET_CONFIG_KEYS.FREE_GENERATIONS_NEW_USER]:
      "Number of free AI generations for new users",
    [WALLET_CONFIG_KEYS.MINIMUM_TOPUP_PAISE]:
      "Minimum wallet top-up amount in paise",
    [WALLET_CONFIG_KEYS.MAXIMUM_TOPUP_PAISE]:
      "Maximum wallet top-up amount in paise",
  };
