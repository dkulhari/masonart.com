...
adminApprovalsApp.get("/", zValidator("query", listApprovalsSchema), async (c) => {
...
adminApprovalsApp.get("/stats", async (c) => {
  try {
    ...
    const recentApproved = await db
      ...
      .from(productionApprovals)
      .where(
        and(eq(productionApprovals.status, "approved"), isNotNull(productionApprovals.approvedAt))
      );
...
adminApprovalsApp.post("/:id/photos", zValidator("json", uploadPhotosSchema), async (c) => {
  const { id } = c.req.param();
  ...
  try {
    ...
    if (!result.success) {
      return c.json({ success: false, error: result.error || "Failed to upload photos" }, 400);
    }
...
adminApprovalsApp.post("/:id/comments", zValidator("json", addCommentSchema), async (c) => {
  const { id } = c.req.param();
  ...
  try {
    ...
    const newComment = await addAdminComment(id, adminUser?.id || "admin", comment);
